...
class SoftDiceLoss(nn.Module):
    ...
    def forward(self, input, target):
        ...
        input = F.softmax(input, dim=1).view(batch_size, self.n_classes, -1)#nb*class* (h*w)
        target = self.one_hot_encoder(target).contiguous().view(batch_size, self.n_classes, -1)#nb*class* (h*w)
...
class My_CE(nn.CrossEntropyLoss):
    def __init__(self, nclass, weight, batch_size):#, reduction='mean'
        super(My_CE, self).__init__(weight = weight)#,reduction=reduction
        self.nclass = nclass
